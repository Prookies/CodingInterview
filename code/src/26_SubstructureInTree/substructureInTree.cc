...
bool HasSubtree(BinaryTreeNode *pRoot1, BinaryTreeNode *pRoot2)
{
    ...
    stack<BinaryTreeNode *> stemp;
    stemp.push(pRoot1);

    BinaryTreeNode *pCur = pRoot1;
    while (!stemp.empty())
    {
        pCur = stemp.top();
        stemp.pop();
        // 注意m_nVal的类型，如果是double类型，其是不能直接比较的
        if (pCur->m_nVal == pRoot2->m_nVal)
        {
            if (DoseTree1HaveTree2(pCur, pRoot2))
            {
                return true;
            }
        }
        ...
        if (pCur->m_pRight)
        {
            stemp.push(pCur->m_pRight);
        }
        if (pCur->m_pLeft)
        {
            stemp.push(pCur->m_pLeft);
        }
    }
