...
void MirrorIteratively(BinaryTreeNode *pRoot)
{
    ...
    while (tempS.empty())
    {
        ...
        if (pCur->m_pLeft || pCur->m_pLeft)
        {
            BinaryTreeNode *tempNode = pCur->m_pLeft;
            pCur->m_pLeft = pCur->m_pRight;
            pCur->m_pRight = tempNode;

            if (pCur->m_pLeft)
            {
                tempS.push(pCur->m_pLeft);
            }

            if (pCur->m_pRight)
            {
                tempS.push(pCur->m_pRight);
            }
        }
    }
}
