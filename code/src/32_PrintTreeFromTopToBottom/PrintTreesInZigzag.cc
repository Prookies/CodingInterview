...
void Print(BinaryTreeNode *pRoot)
{
    ...
    deque<BinaryTreeNode *> dequeTemp;
    dequeTemp.emplace_back(pCur);

    int nLevel = 1;

    while (dequeTemp.size())
    {

        int nLen = dequeTemp.size();

        for (int i = 0; i < nLen; i++)
        {
            if (nLevel & 1)
            {
                pCur = dequeTemp.front();
                dequeTemp.pop_front();

                if (pCur->m_pLeft)
                {
                    dequeTemp.push_back(pCur->m_pLeft);
                }

                if (pCur->m_pRight)
                {
                    dequeTemp.push_back(pCur->m_pRight);
                }
            }
            else
            {
                ...
                if (pCur->m_pRight)
                {
                    dequeTemp.push_front(pCur->m_pRight);
                }

                if (pCur->m_pLeft)
                {
                    dequeTemp.push_front(pCur->m_pLeft);
                }
            }
