...
const BinaryTreeNode *KthNode(const BinaryTreeNode *pRoot, unsigned int k)
{
    ...
    stack<const BinaryTreeNode *> pNodes;
    const BinaryTreeNode *pCur = pRoot;
    ...
    const BinaryTreeNode *pResult = nullptr;
    int count = 0;
    while (!pNodes.empty())
    {
        const BinaryTreeNode *pNode = pNodes.top();
        pNodes.pop();
        count++;
        if (count == k)
        {
...
void Test(const char *testName, const BinaryTreeNode *pRoot, unsigned int k, bool isNull, int expected)
{
    ...
    if ((isNull && pTarget == nullptr) || (!isNull && pTarget->m_nVal == expected))
        printf("Passed.\n");
    else
        printf("FAILED.\n");
}
